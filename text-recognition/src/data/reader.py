...
class Dataset():
    ...
    def _dpeter(self):
        ...
        for partition in ['train', 'valid']:
            with open(os.path.join(self.source, f'data/{partition}.json')) as f:
                for line in f:
                    path_to_image_and_words = json.loads(line)
                    image_path = path_to_image_and_words["image_path"]
                    words_path = path_to_image_and_words["text_path"]
                    with open(os.path.join(self.source, words_path)) as f:
                        dataset[partition]['gt'].append(f.read().strip())
                    dataset[partition]['dt'].append(os.path.join(self.source, image_path))
